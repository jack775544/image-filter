...
def get_designated_folder(img, day=0):
    ...
    root = img[2]
    if day == 1:
        return os.path.join(get_higher_dir(get_higher_dir(root)), "sorted",
                            "night", "images " + get_monday(time)
                            .strftime("%Y %m %d"))
    else:
        return os.path.join(get_higher_dir(get_higher_dir(root)), "sorted",
                            "day", "images " + get_monday(time)
                            .strftime("%Y %m %d"))
...
def main():
    ...
    folder = get_folder(filedialog.askdirectory(
        initialdir="/", title="The root folder of the unsorted images"))
    print(folder)
    acknowledge = input("press y then enter to continue: ")
    ...
    for pic in piclist:
        x = x + 1
        start_time = datetime.datetime.strptime(str(pic[1].year) + ":" + str(
            pic[1].month) + ":" + str(pic[1].day) + " 7:30:00",
            "%Y:%m:%d %H:%M:%S")
        end_time = datetime.datetime.strptime(str(pic[1].year) + ":" + str(
            pic[1].month) + ":" + str(pic[1].day) + " 16:30:00",
            "%Y:%m:%d %H:%M:%S")
